...
export interface FishBehaviorOptions {
  ...
  restDuration?: number;         // how long the fish remains in REST state (in seconds) after eating
  ...
  restCheckInterval?: number;    // How often (in seconds) to check if the fish should rest
...
export class FishBehavior {
  ...
  public update(headPosition: THREE.Vector3, velocity: THREE.Vector3, deltaTime: number) {
    switch (this.state) {
      ...
      case FishState.WANDER:
        // If fish is talking, it should not make autonomous decisions like resting.
        // Actual movement during TALK is handled by Fish.tsx based on velocity when TALK started.
        if (this.isTalking) {
            break;
        }
        ...
        // Priority 2: Periodically check if the fish should rest.
        ...
        // Otherwise, continue wandering (actual movement logic is in Fish.tsx)
        break;

      case FishState.TALK:
        // While talking, the fish might be stationary or continue its previous movement.
        // Don't change any movement behavior while talking
        // This allows the fish to continue its current movement
        break;
    }
  }
  ...
  // Updated to not make the fish stationary
  public startTalking(position: THREE.Vector3, velocity: THREE.Vector3, message?: string) {
    // Enter stationary state with TALK state (makes fish stop moving)
  ...
  public stopTalking() {
    if (this.state === FishState.TALK) {
      ...
      this.state = FishState.WANDER; // Default to WANDER after talking
