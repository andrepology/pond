...
import { useFrame, RootState, ThreeEvent, useThree } from '@react-three/fiber'
import { Html,  OrbitControls } from '@react-three/drei'
import * as THREE from 'three'
import { useControls,  } from 'leva'
import { EffectComposer, Bloom} from '@react-three/postprocessing'
import { FishBehavior, FishState } from '../../steering/FishBehavior'
import { FishMessageContext } from '../LogInterface'
import { useDevice } from '../../contexts/DeviceContext'
import { useSpring, a } from '@react-spring/three'
import { useAccount } from 'jazz-react'
import { invokeInnio } from '../../hooks/invokeInnio'
...
const Fish: React.FC<FishProps> = ({ onPositionUpdate }) => {
  ...
  const movementControls = useControls('Movement', {
    maxSpeed: { value: 0.01, min: 0.01, max: 0.1, step: 0.01 },
  ...
  // Add this near the top of your Fish component, with other hooks/refs
  ...
  // Debug logger for vector pool usage
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      const intervalId = setInterval(() => {
        console.log(`Vector pool stats: ${vectorPool.size()} available, ${vectorPool.createdCount()} created when depleted`);
      }, 10000);
  ...
  const updateMovement = (delta: number) => {
    ...
    if ((fishBehavior.state === FishState.REST || fishBehavior.state === FishState.TALK) 
      ...
      perp.set(-fishBehavior.stationaryDirection.z, 0, fishBehavior.stationaryDirection.x);
      
      // Get targetPos from pool
      const targetPos = vectorPool.get();
      targetPos.copy(fishBehavior.stationaryPosition).add(perp.multiplyScalar(sway));
      ...
      // Get desired vector from pool
      const desired = vectorPool.get();
      desired.subVectors(fishBehavior.target, headRef.current!.position);
      const dist = desired.length();
      desired.normalize();
      ...
      console.log("Fish in TALK state - should keep moving");
  ...
  const computeTargetDirection = (): THREE.Vector3 => {
    ...
    if ((fishBehavior.state === FishState.REST || fishBehavior.state === FishState.TALK) 
        && fishBehavior.stationaryDirection) {
      candidateTmp.copy(fishBehavior.stationaryDirection);
    } else if (fishBehavior.state === FishState.APPROACH && fishBehavior.target) {
      candidateTmp.copy(fishBehavior.target).sub(headRef.current!.position);
  ...
  // Modify the fish spring to include a loading animation
  ...
  // Update the handleFishClick function
  const handleFishClick = useCallback(async (/* e: ThreeEvent<MouseEvent> */) => {
    ...
    try {
      ...
      if (response && headRef.current) {
        console.log('🐠 Fish received response:', response.substring(0, 50) + '...');
      ...
      console.error("Error getting fish response:", error)
      
      // Optional: make fish say an error message
      if (headRef.current) {
        fishBehavior.startTalking(
          headRef.current.position.clone(),
          currentVelocity.current.clone(),
  ...
  }, [fishBehavior, me, generateResponse, setHeadSpring, resetForNewEntry]) // Added resetForNewEntry to dependencies
  ...
  useFrame((state: RootState, delta: number) => {
    ...
    //บาท Synchronize hitbox position with the head
  ...
  // Refs for timeouts
  const wordVisibilityTimeoutsRef = useRef<NodeJS.Timeout[]>([]);
  const stopTalkCleanupTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  ...
  useEffect(() => {
    ...
    if (fishBehavior.state === FishState.TALK) {
      ...
      stopTalkCleanupTimeoutRef.current = setTimeout(() => {
        // Double-check state in case fish started talking again very quickly
        if (fishBehavior.state !== FishState.TALK) {
  ...
  useEffect(() => {
    ...
    const startAnimationTimeout = setTimeout(() => {
      ...
      const newTimeoutsHolder: NodeJS.Timeout[] = [];
  ...
  // Effect to handle incoming messages and make fish talk
  useEffect(() => {
    console.log("Fish message changed:", fishMessage);
    console.log("Current fish state:", fishBehavior.state);
    
    if (fishMessage && fishBehavior.state !== FishState.TALK) {
      // Start talking with the new message
      if (headRef.current) {
        console.log("Starting fish talking");
        fishBehavior.startTalking(
          headRef.current.position.clone(),
          currentVelocity.current.clone(),
          fishMessage
  ...
  // Function to handle feeding the fish and create ripple
  const handleFeedFish = (position: THREE.Vector3) => {
    const pt = position.clone()
    pt.y = 0
    
    // Only set food target if the queue isn't too full
    if (fishBehavior.targetQueue.length <= 10) {
  ...
  const CLICK_MOVE_THRESHOLD_SQUARED = 5 * 5; // Max movement squared for a tap/click
  ...
  return (
    <FishMessageContext.Provider value={{ setFishMessage }}>
            ...
            if (!isMobileDevice) {
                ...
                if (timeSinceLastTap < DOUBLE_CLICK_TIME_THRESHOLD) {
                 ...
                 if (currentPressStartTime && Date.now() - currentPressStartTime > longPressThreshold) {
                     if (downDetails.point) {
                         handleFeedFish(downDetails.point);
                     }
                 }
            }
          }}
          onPointerMove={(e: ThreeEvent<PointerEvent>) => {
             if (pressPosition && pressPosition.distanceTo(e.point) > 0.5) {
                setPressStartTime(null)
                setPressPosition(null)
             }
           }}
